...
import { 
  ...
  addEnergyReading, deleteEnergyReading,
    ...
    <div className="min-h-screen bg-[#FDFCFB] dark:bg-stone-950 text-stone-900 dark:text-stone-100 pb-40 font-sans selection:bg-[#C5A38E]/30">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 mt-6 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-[#C5A38E] text-white p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-[10px] text-white/60 mt-1 uppercase tracking-widest">(Synchronisiert aus Weltkarte)</p>
            </div>
          ...
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-[2.5rem] p-6 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="space-y-3">
                {energyReadings.length === 0 && <p className="text-xs text-stone-400 italic">Noch keine Zählerstände.</p>}
                {energyReadings.map(r => (
                  <div key={r.id} className="flex justify-between items-center text-sm border-b border-stone-100 dark:border-stone-800 pb-2 group">
                    <span className="text-stone-500 text-xs">{new Date(r.date).toLocaleDateString('de-DE', { month: 'short', day: '2-digit' })}</span>
                    <span className="font-bold tabular-nums">{r.value} <span className="text-[10px] text-stone-400 font-normal">kWh</span></span>
                    <div className="flex items-center gap-2">
            ...
            <form action={async (formData) => { "use server"; await addEnergyReading("STROM", parseFloat(formData.get("val") as string)); }} className="mt-4 flex gap-2">
              <input name="val" type="number" step="0.1" placeholder="Zählerstand (kWh)..." className="flex-1 h-12 bg-stone-50 dark:bg-stone-950 px-4 rounded-xl text-xs outline-none" required />
              <button className="px-4 h-12 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold transition-colors">Track</button>
            </form>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* SCHWARZES BRETT (BASE64 IMAGE FIX) */}
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
              ...
              {stickyNotes.map(note => (
                <div key={note.id} className="bg-stone-50 dark:bg-stone-800/40 p-4 rounded-3xl border border-stone-100 dark:border-stone-800 relative group">
                  ...
                  {/* Base64 Bilder werden hier direkt und nativ angezeigt */}
                  {note.imageUrl && (
